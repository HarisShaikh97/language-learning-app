...
export default function LanguageCard({ id, name, image, href }) {
	...
	return (
		...
		>
			<div className="h-40 w-full bg-sky-200 relative flex flex-col items-start gap-5 p-5">
				<p className="text-xl font-bold text-primary">{name}</p>
				<p className="font-semibold text-primary">Teacher name</p>
				<div
					className="size-20 rounded-full bg-cover bg-no-repeat bg-center absolute -bottom-6 right-16"
